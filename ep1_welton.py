...
prpscao = []
for prep in range(0,len(txtA)):
	if txtA[prep][0] in 'zmb' and txtA[prep][-1] not in 'zmb':
		prpscao.append(txtA[prep])

print ("QUESTÃO A: no Texto A, contém %d preposições" % len(prpscao))

verbos = []

for v in range(0,len(txtA)):
	if len(txtA[v]) == 7:
		verbos.append(txtA[v]) # Lista com todos os verbos, palavras com 7 letras

contador = 0

vp1 = []

for v1 in range(0,len(verbos)):
	if (verbos[v1][-1]) not in 'zmb' and (verbos[v1][0]) not in 'zmb':
		vp1.append(verbos[v1])
		
print ("QUESTÃO B: no Texto A, contém %d verbos" % len(verbos))
print ("QUESTÃO C: no Texto A, contém %d verbos na primeira pessoa" % len(vp1))

lexico = 'zmbtshjpnwlrcxkqvdgf'
for ordena in lexico:
	for googlon in txtA:
		if txt
